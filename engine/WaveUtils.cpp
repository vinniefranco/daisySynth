...
// fillTables2:
//
// Alternate version that allows you specify to minumum  maximum harmonic
// coverage. minTop: the minimum normalized frequency that all wave tables
// support
//      ex.: 18000/44100.0 ensures harmonics out to 18k (44.1kHz sample rate) at
//      minimum
// maxTop: the maximum normalized freuqency that all wave tables support
//      ex.: 0.5 give full bandwidth without aliasing; 24000/44100.0 allows a
//      top of 24k, some aliasing
// The function fills the oscillator with all wavetables necessary for
...
int fillTables2(WaveTableOsc *osc, double *freqWaveRe, double *freqWaveIm,
  ...
  while (maxHarmonic) {
    ...
    while ((abs(freqWaveRe[maxHarmonic]) + abs(freqWaveIm[maxHarmonic]) <
           ...
           maxHarmonic)
      --maxHarmonic;
    float topFreq = maxTop / maxHarmonic;
    ...
    // topFreq is new base frequency, so figure how many harmonics will fit
    // within maxTop
