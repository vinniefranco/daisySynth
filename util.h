...
// If input - _limit < 0, then it really substracts, and the 0.5 to make it half
// the 2 inputs. If > 0 then they just cancel, and keeps input normal. The
// easiest way to understand it is check what happends on both cases.
...
inline float clip(float value, float saturation, float saturationinverse) {
  float v2 =
      (value * saturationinverse > 1
           ? 1
           : (value * saturationinverse < -1 ? -1 : value * saturationinverse));
